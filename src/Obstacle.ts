 ...
 */
export function buildCurrents(targetAltitude: number): Current[] {
 ...
 */
export function buildPlankton(): Plankton[] {
  const plankton: Plankton[] = [];

  for (let index = 0; index < 58; index += 1) {
    const y = -130 - index * 60;
    const x = Math.sin(index * 2.1) * 430 + Math.cos(index * 0.65) * 80;
    plankton.push({
      x,
      y,
      baseX: x,
